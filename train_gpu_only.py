#!/usr/bin/env python3
"""
FinAI GPU-Only Training Script
Creates a new model specifically optimized for DirectML GPU training
"""
import os
import sys
import time
...
def __init__(self):
# GPU-specific model paths
self.MODEL_DIR = "models"
self.LANGUAGE_MODEL_PATH = f"{self.MODEL_DIR}/finai_gpu.pt" # Separate GPU model
self.TOKENIZER_PATH = f"{self.MODEL_DIR}/tokenizer_gpu.pkl" # Separate GPU tokenizer
self.DATASET_DIR = "datasets"

# GPU-optimized architecture (very small for DirectML stability)
self.BLOCK_SIZE = 128 # Reduce context size
self.N_LAYER = 1 # Minimal layers
self.N_HEAD = 2 # Fewer heads
self.N_EMBD = 128 # Smaller embeddings
self.DROPOUT = 0.05

# GPU training settings
self.TRAIN_EPOCHS = 1
self.TRAIN_STEPS = 5000
self.BATCH_SIZE = 2 # Very small batch for DirectML
self.GRADIENT_ACCUM_STEPS = 32 # Keep effective batch ~64
...
# Other settings
self.USE_GRAD_CHECKPOINTING = False # Disable for DirectML stability
...
print(f"\n{'='*80}")
print(f"Training on dataset {dataset_number}: {dataset_name} [GPU-Only Model]")
print("="*80)
try:
...
temp_file = f"datasets/temp_gpu_{dataset_number}.txt"
...
gc.collect()
DEVICE = get_device()
print(f"[FinAI] Fresh GPU device: {DEVICE}")
except Exception as e:
print(f"[FinAI] Device reset failed: {e}")

print(f"\n{'='*80}")
print(f"Training on {dataset_name} [GPU-Only Model]...")
print(f"{'='*80}\n")

# Create fresh FinAI instance with GPU config
finai = FinAI()
# Override config paths to use GPU-specific model
finai.config.LANGUAGE_MODEL_PATH = cfg.LANGUAGE_MODEL_PATH
finai.config.TOKENIZER_PATH = cfg.TOKENIZER_PATH
finai.config.N_LAYER = cfg.N_LAYER
finai.config.N_HEAD = cfg.N_HEAD
finai.config.N_EMBD = cfg.N_EMBD
finai.config.BLOCK_SIZE = cfg.BLOCK_SIZE
finai.config.BATCH_SIZE = cfg.BATCH_SIZE
finai.config.GRADIENT_ACCUM_STEPS = cfg.GRADIENT_ACCUM_STEPS
finai.config.USE_GRAD_CHECKPOINTING = cfg.USE_GRAD_CHECKPOINTING

print(f"[FinAI] GPU Model Path: {cfg.LANGUAGE_MODEL_PATH}")
print(f"[FinAI] Training device: {DEVICE}")
...
mixed_precision='no', # Disable mixed precision for DirectML
...
def main():
print("\n" + "="*80)
print("FinAI - GPU-Only Training (Separate GPU Model)")
print("="*80 + "\n")
...
print("\n" + "="*80)
print("GPU-Only training completed!")
print(f" Total datasets trained: {dataset_number - len(trained_datasets) - 1}")
print(f" GPU model saved at: {cfg.LANGUAGE_MODEL_PATH}")
print("="*80 + "\n")
