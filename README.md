...
## Training with AMD GPU on Windows (DirectML)

This project now supports AMD GPUs for training on Windows using Microsoft's DirectML backend, in addition to the original Hugging Face Accelerate method. These new scripts are specifically made for AMD Radeon GPUs (e.g., 7600XT) and do not require CUDA or NVIDIA hardware.

### 1. Optional: Create a Python virtual environment
```bash
python -m venv directml_env
# Activate the environment (PowerShell):
.\directml_env\Scripts\Activate.ps1
# Or with CMD
.\directml_env\Scripts\activate.bat
```

### 2. Install DirectML-enabled PyTorch and dependencies
```bash
pip install torch-directml
datasets
```
...
### 3. Run training with new DirectML scripts
- To train on **all datasets at once** (unified model):
```bash
python train_all_directml.py
```
- To train **sequentially**, one dataset at a time:
```bash
python train_sequential_directml.py
```
- To train on a **single dataset**:
```bash
python train_single_directml.py <dataset_name>
```
_(e.g., python train_single_directml.py vumichien/financial-sentiment)_

#### What happens?
- The scripts will auto-select the best GPU backend: CUDA (NVIDIA, if found), DirectML (AMD/Windows), or CPU (fallback). Training speed is far higher with a supported GPU.

### 4. Troubleshooting
- If `torch-directml` does **not** install, follow Windows setup instructions at:
https://learn.microsoft.com/en-us/windows/ai/directml/pytorch-windows
- If you face any issues, you may still use the **original** scripts (`train_all.py`, etc.) which rely on Hugging Face Accelerate (see below). These fall back to CPU or CUDA-based GPUs and do not use DirectML/AMD.

### 5. Legacy Accelerate Scripts
If DirectML has compatibility issues, you may still invoke the legacy scripts with:
```bash
pip install accelerate torch
accelerate launch train_all.py
# Or: accelerate launch train_sequential.py
```

**Note:** If you are using an NVIDIA GPU, CUDA will be auto-detected; if AMD + Windows, DirectML will be selected if possible; otherwise, CPU is used. DirectML is only available on Windows 10+ with supported AMD drivers.

For further help, see the troubleshooting guide in [REMOTE_ACCESS_SETUP.md] or raise an Issue.