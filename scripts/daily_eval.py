...
def save_result(prompt: str, response: str, model_params: str, history_file: str = "daily_eval_history.json"):
...
def main():
    ...
    parser.add_argument(
        "--hf-repo",
        type=str,
        help="Hugging Face repo ID (alternative to --model-path)"
    )
    parser.add_argument(
        "--prompt",
        type=str,
    ...
    try:
        ...
        try:
            ...
            model_params = str(config.num_parameters) if hasattr(config, 'num_parameters') else "unknown"
        except:
