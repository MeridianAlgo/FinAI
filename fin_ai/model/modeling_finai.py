...
class FinAIAttention(nn.Module):
    ...
    def forward(
        ...
        key_states = torch.repeat_interleave(key_states, self.n_heads // self.n_kv_heads, dim=1)
        value_states = torch.repeat_interleave(value_states, self.n_heads // self.n_kv_heads, dim=1)
        ...
        if self.config.use_flash_attention:
            ...
            if attention_mask is None and q_len > 1:
                casual_mask = torch.triu(torch.ones(q_len, kv_seq_len, dtype=torch.bool, device=hidden_states.device), diagonal=1)
                attn_weights.masked_fill_(casual_mask, torch.finfo(attn_weights.dtype).min)
...
class FinAIModel(FinAIPreTrainedModel):
    def __init__(self, config: FinAIConfig):
        ...
        self.layers = nn.ModuleList([FinAIBlock(config) for _ in range(config.n_layers)])
